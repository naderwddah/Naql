...
class StatusController {
    ...
    public function create($data) {
        ...
        if (!$name) {
            Response::error("اسم الحاله مطلوب", 400);
        }
        ...
        try {
            ...
            if ($e->errorInfo[1] == 1062) {
                Response::error("اسم الحاله موجود", 409);
            }
            throw $e;
        }
    }
    ...
    public function update($data) {
        ...
        if (!$status_id || !$name) {
            Response::error("رقم الحه واسم الحاله مطلوب", 400);
        }

        // منع تعديل الحالات النظامية
        if (in_array($status_id, [1, 2, 3, 4])) {
            Response::error("لايومكنك تعديل حالات النظام الاساسية", 403);
        }
        ...
        Response::success(['message' => 'تم تحديث الحاله']);
    }
    ...
    public function delete($data) {
        ...
        $status_id = $data['status_id'] ?? null;
        if (!$status_id) {
            Response::error("معرف الحاله مطلوب", 400);
        }

        // منع حذف الحالات النظامية
        if (in_array($status_id, [1, 2, 3, 4])) {
            Response::error("لا يمكنك حذف حالات النظام الاساسية", 403);
        }
        ...
        Response::success(['message' => 'تم حذف الحالة']);
    }
